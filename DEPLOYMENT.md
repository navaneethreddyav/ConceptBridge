...
4. **A local terminology dataset pre-filters concept detection before Gemini is ever called.** `shared/terminologyDataset.json` (~346 curated engineering terms across CS, EEE, ECE, Mechanical, and Civil, grounded in Osmania University's Faculty of Engineering curriculum — see `shared/terminologyDataset.SOURCES.md` for exact sourcing) is matched against the document sample with zero network calls in `backend/src/services/terminology/terminologyMatcher.js`. If that alone finds at least 6 distinct terms, `conceptDetectionService.js` returns them directly and skips the Gemini call entirely for that document. Below that threshold, Gemini still runs as before and the dataset matches are merged into its result. Explaining a selected term (the actual "select → get an explanation" flow) always calls Gemini regardless of how the term was detected — only the auto-highlight detection pass is affected.
